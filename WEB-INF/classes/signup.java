...
public class signup extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            Connection conn = DriverManager.getConnection("jdbc:mysql://localhost/todo", "root", "");
            Statement stmt = conn.createStatement();
            ResultSet userdata;

            userdata = stmt.executeQuery("SELECT * FROM login_info WHERE user_email = '" + user_email + "';");

            if (userdata.next()) {
                out.println("<script>alert('Email Already In Use'); window.location.href='signup.jsp';</script>");
            }
            else{
                userdata = stmt.executeQuery("SELECT * FROM login_info WHERE user_name = '" + username + "';");

                if (userdata.next()) {
                    out.println("<script>alert('Username Already In Use'); window.location.href='signup.jsp';</script>");
                }
                else {
                    PreparedStatement pstmt = conn.prepareStatement("INSERT INTO login_info VALUES (?,?,?);");
